...
```bash
node ~/.claude/skills/visual-feedback/scripts/receiver.js
```
...
3. 点「加载已解压的扩展程序」，选择目录：
   ```
   ~/.claude/skills/visual-feedback/extension/
   ```
...
> ```
> 将 `~/.claude/skills/visual-feedback/extension/inject.js` 全部内容复制粘贴到 Console 回车。
...
```bash
pkill -f "visual-feedback/scripts/receiver.js"
```
告诉用户「接收端已关闭」。
